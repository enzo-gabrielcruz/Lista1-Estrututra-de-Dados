...
void PreencherVetor(const char *valor, int *qtd_int, int *qtd_str, int *qtd_real, int *qtd_ponto,
                    int *inteiros, float *reais, char **strings, char **pontos) {
    ...
    else if (tipo == 4) pontos[(*qtd_ponto)++] = strdup(valor);
}
...
void printer(FILE *out, int qtd_str, char **strings, int qtd_int, int *inteiros, int qtd_real, float *reais, int qtd_ponto, char **pontos) {
    fprintf(out, "str: ");
    ...
    fprintf(out, "p: ");
    for(int i = 0; i < qtd_ponto; i++){
        if (i < qtd_ponto -1 )fprintf(out, "%s ", pontos[i]);
        if (i == qtd_ponto -1)fprintf(out,"%s",pontos[i]);
    } fprintf(out, "\n");
}
...
int main() {
    ...
    while (fgets(line, LINE_MAX_SIZE, file_in)) {
        ...
        qsort(pontos, qtd_ponto, sizeof(char *), cmp_str);

        printer(file_out, qtd_str, strings, qtd_int, inteiros, qtd_real, reais, qtd_ponto, pontos);

        for (int i = 0; i < qtd_str; i++) free(strings[i]);
        for (int i = 0; i < qtd_ponto; i++) free(pontos[i]);
    }
